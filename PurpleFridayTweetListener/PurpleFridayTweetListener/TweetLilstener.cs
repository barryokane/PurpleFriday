...
namespace PurpleFridayTweetListener
{
    public class TweetListener
    {
        ...
        public void StartStream(string filter)
        {
            ...
            stream.MatchingTweetReceived += async (sender, targs) =>
            {
                ...
                if (targs.Tweet.Place != null)
                {
                    ...
                    await HandleTweetWithLocationData(targs.Tweet);
                    forwardedTweetData = true;
                }
                else
                {
                    await HandleTweetWithoutLocationData(targs.Tweet, filter);
                }
        ...
        protected async Task HandleTweetWithLocationData(ITweet tweet)
        {
            ...
            await ForwardTweetData(tweetData);
        }

        protected async Task HandleTweetWithoutLocationData(ITweet tweet, string filterHashtag)
        {
        ...
        private async Task ForwardTweetData(TweetData data)
        {
